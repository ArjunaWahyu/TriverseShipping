...
exports.register = (req, res) => {
    ...
    mysqlConnection.query('SELECT email FROM users WHERE email = ?', [email], async (error, results) => {
        ...
        if (results.length > 0) {
            ...
            return ejs.renderFile(path.join(__dirname, "../pages/register.html"), {
...
exports.login = (req, res) => {
    ...
    mysqlConnection.query('SELECT * FROM users WHERE name = ?', [name], async (error, results) => {
        ...
        ejs.renderFile(path.join(__dirname, "../pages/home.html"), { userLoggedIn, username, userEmail }, (err, data) => {
            if (err) {
                console.error("Error rendering HTML:", err);
                return res.status(500).send('Internal Server Error');
            }
